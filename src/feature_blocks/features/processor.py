...
from feature_blocks.image import tissue_detection, normalise_rgb
from feature_blocks.features import transformer_features, feature_map_blocks2, feature_map_overlap_blocks, postprocess_dask_transformer_features
from feature_blocks.cluster import cluster_blocks
...
def get_vit_patch_features_and_clusters(
    ...
    if create_mask:
        mask = tissue_detection(image)

    img_norm = normalise_rgb(image, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ...
    if overlap is not None:
        feature_blocks = feature_map_overlap_blocks(
            img_norm,
            feature_extract_function=transformer_features,
            mask=mask,
    ...
    patch_features, cls_features = (
        postprocess_dask_transformer_features(feature_blocks)
    )
    ...
    patch_clusters, patch_cmap = cluster_blocks(
    ...
    if calculate_cls_clusters:
        ...
        cls_clusters, cls_cmap = cluster_blocks(
