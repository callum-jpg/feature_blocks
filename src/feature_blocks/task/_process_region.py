...
def process_region_batch(
    ...
    """Process a batch of regions: read -> infer -> write for each region in the batch

    This reduces task overhead by processing multiple regions in a single task.

    Args:
        region_batch: List of region tuples (slices or with chunk_id/mask_index)
        input_zarr_path: Path to input zarr
        model_identifier: String model name (serialization-friendly) or callable
        n_features: Number of features the model produces
        output_zarr_path: Path to output zarr
