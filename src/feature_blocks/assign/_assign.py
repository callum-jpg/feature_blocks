...
def assign_cell_cluster_block(
    ...
    original_image = get_spatial_element(
        getattr(sdata, "images"), sdata_image_key, as_spatial_image=True
    )
...
def scale_coords(
    xy: numpy.ndarray, original_shape, new_shape, extra_scale_factor=1 / 0.2125
) -> numpy.ndarray:
    ...
    xy = numpy.multiply(
        xy, [x_scale * extra_scale_factor, y_scale * extra_scale_factor]
    ).astype(int)
